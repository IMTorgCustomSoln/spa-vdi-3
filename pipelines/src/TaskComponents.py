...
class ExportIndividualPdfTask(Task):
    ...
    def run(self):
        ...
        for file in processed_files:
            ...
            self.config['LOGGER'].info("begin export")
            record = file.load_file(return_content=True)
            pdf_bytes = uint8array_to_pdf_file(record.presentation_doc['file_uint8arr'])
            #export
            ...
            if check:
                cnt = cnt + 1
                record.presentation_doc['export_id'] = f'{file.filepath.stem}.pdf'
                record.presentation_doc['printed'] = True
            else:
                record.presentation_doc['printed'] = False
            report_records.append(record.presentation_doc)
